...
class RecDataset(object):
    def __init__(self, config, df=None):
        ...
        cur_dir = os.getcwd()
        cur_dir = os.path.join(cur_dir, config['data_path'])
        self.dataset_path = os.path.join(cur_dir, self.dataset_name)
    ...
    def split(self):
        ...
        if self.config['no_valid']:
            for i in range(3):
                if i==2:
                    temp_df = self.df[self.df[self.splitting_label].isin([1,2])].copy()
                    dfs.append(temp_df)
                else: 
