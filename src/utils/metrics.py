...
def mrr_(pos_index, pos_len):
    r"""MRR_ (Mean Reciprocal Rank) is a statistic measure for evaluating any process that produces a list of possible
    ...
    .. math::
        \mathrm{MRR@K} = \frac{1}{|U|} \sum_{u=1}^{|U|} \frac{1}{rank_u}
    
    :math:`rank_u` is the position of the first relevant item for user :math:`U`
    """
    user_num, max_k = pos_index.shape
    ranks = np.arange(1, max_k + 1).reshape(1, -1)  
    reciprocal_ranks = np.where(pos_index, 1 / ranks, 0)  
    result = np.cumsum(reciprocal_ranks, axis=1)  
    return result.mean(axis=0)  


def hit_(pos_index, pos_len):
    r"""Hit Rate (HR) measures whether at least one relevant item appears in the recommended list.
    
    .. math::
        \mathrm{HR@K} = \frac{| \{ u | \exists i \in Rec_u \cap Rel_u \} |}{|U|}
    
    It checks if at least one of the recommended items is relevant.
    """
    hit_scores = np.cumsum(pos_index, axis=1) > 0  
    result = hit_scores.mean(axis=0)  
    return result
