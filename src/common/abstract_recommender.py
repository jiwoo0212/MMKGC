...
class GeneralRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataloader):
        ...
        if not config['end2end'] and config['is_multimodal_model']:
            ...
            if config['sub_folder_name']:
                dataset_path = os.path.join(dataset_path, config['sub_folder_name'])
            ...
            if config['use_review']:
                r_feat_file_path = os.path.join(dataset_path, config['review_feature_file'])
                if os.path.isfile(r_feat_file_path):
                    self.r_feat = torch.from_numpy(np.load(r_feat_file_path, allow_pickle=True)).type(torch.FloatTensor).to(
                        self.device)
            if config['use_llm']:
                l_feat_file_path = os.path.join(dataset_path, config['llm_feature_file'])
                if os.path.isfile(l_feat_file_path):
                    self.l_feat = torch.from_numpy(np.load(l_feat_file_path, allow_pickle=True)).type(torch.FloatTensor).to(
                        self.device)
